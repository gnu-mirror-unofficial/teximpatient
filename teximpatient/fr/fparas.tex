...
Ces commandes produisent divers marques et caract\`eres sp\'eciaux. Les 
...
\^etre cr\'enel\'ee ou remplac\'ee par une ligature. (aucunes polices 
standards de \TeX\ ne contient ce caract\`ere de fronti\`ere.) L'effet de 
...
tokens de la liste qui ne sont pas des lettres ne sont pas affecter---m\^eme 
...
\endolist
\noindent
Cependant, |^^M| ne fait \emph{pas} terminer la ligne quand il 
...
Cependant,|\obeyspaces| ne met pas d'espaces au d\'ebut d'une ligne~; 
...
un source informatique par exemple, dans une police \`a espacement fixe (une dans 
...
Pendant que \TeX\ traite une \minref{liste horizontal}, il garde trace 
...
\endulist
Voir \knuth{page~76}{99} pour les r\`egles pr\'ecises que \TeX\ utilise pour 
calculer le \minref{ressort} entre les mots et \knuth{pages~285--287}{999-999} 
pour les ajustements fait \`a |\spacefactor| apr\`es diff\'erents items dans 
...
\explain
La commande |\centerline| produit un \minref{hbox} aussi au large que 
...
\explain
Cette commande produit un \minref{hbox} contenant l'\<argument>. Cette 
hbox est exactement aussi large que la ligne courante, c'est-\`a-dire, 
qu'il s'\'etend de la marge droite \`a la gauche.
\example
...
cr\'eant un \minref{box} de largeur z\'ero.
...
Puisque \TeX\ convertis une ligne blanche dans votre fichier source 
...
produit avec |\par| en mettant la commande |\vskip -\lastskip|. Caci 
...
Voir \knuth{pages~283 et 286}{999 et 999} pour les effets pr\'ecis de |\par|.
...
ligne d'un paragraphe sur la marge droite. 
\enddesc
...
\easy\cts indent {}
\explain
Si \TeX\ est en mode verticale, comme il l'est apr\`es avoir fini un 
paragraphe, cette commande ins\`ere le ressort inter-paragraphe 
...
\explain
Ce \minref{param\`etre} indique la longueur courante de la ^{ligne}, c'est-\`a-
dire, la largeur habituelle des lignes dans un paragraphe commen\c cant \`a la 
marge gauche. De grands beaucoup de \TeX \ commandes, par exemple, 
|\centerline| (\xref{\centerline}) et |\hrule| (\xref{\hrule}), emploient 
implicitement la valeur du |\hsize|. En changeant le |\hsize| chez un 
...
gauches et \`a droites de chaque ligne du paragraphe courant. Nous n'expli\-%
...
leur taille naturelle, c'est-\`a-dire, elles ont toutes la m\^eme largeur et
...
cela \'evite des ``rivi\`eres'' distractive d'espace sur la page imprim\'ee.
...
Les $i$ et les $l$ sont tous des dimensions. La premi\`ere ligne est indent\'ee de
...
normalement faire beaucoup d'essai, ins\'erer des cr\'enages ici et l\`a et
...
noter tr\`es tr\`es bons~:  
1962, 1964, 1966. 1958, 1959, 1960, 1961, 1964, 1966 sont de bons crus 
californien. \`A boire avec mod\'eration~! \endexample
\eix^^{indentation}
\enddesc
...
paragraphe.
Voir la \knuth{page~103}{999} pour plus de d\'etails \`a son propos.
\enddesc
...
Une p\'enalit\'e de $10000$ ou plus emp\^eche une coupure quel qu'elle soit,
...
entre des lignes contr\^ol\'ee par |\obeylines| (puisqu'il pense que chaque
...
des coupure de ligne sans c\'esure~;
...
bonne que possible selon les circonstances.
\enddesc
...
d'augmenter la quantit\'e de texte que vous pouvez placer que une page.
...
comme \'eviter des espacements inter-mots excessivement res\-serr\'e.
...
\PlainTeX\ mets |\linepenalty| \`a $10$.
\enddesc
...
\endolist
Deux lignes adjacentes sont visuellement incompatible
...
vous devez assigner une valeur plut\^ot large \`a ce param\`etre (dans les
milliers) pour qu'elle ait un quelconque effet.
\PlainTeX\ mets |\adjdemerits| \`a~$10000$.
\enddesc
...
\PlainTeX\ mets |\exhyphenpenalty| \`a $50$.
\enddesc
...
\PlainTeX\ mets |\hyphenpenalty| \`a $50$.
\enddesc
...
\minrefs{line break}
{\tighten
Ce param\`etre sp\'ecifie des \minref{d\'em\'erites} additionnels que \TeX\
attache \`a un point de coupure quand ce point de coupure arrive \`a
deux lignes cons\'ecu\-tives qui se termines sur une c\'esure.
Augmenter la valeur de ce param\`etre a l'effet de d\'ecourager
\TeX\ de c\'esurer deux lignes \`a suivre.
Les d\'em\'erites sont en unit\'es de \minref{m\'ediocrit\'e} au carr\'e, donc
vous devez assigner une valeur plut\^ot large \`a ce param\`etre (dans les
milliers) pour qu'elle ait un quelconque effet.
\PlainTeX\ mets |\doublehyphendemerits| \`a $10000$.
}
\enddesc
...
la rend m\'eprisable.
Augmenter la valeur de ce param\`etre a le effet de d\'ecourager
\TeX\ de finir l'avant derni\`ere ligne avec une c\'esure.
Les d\'em\'erites sont en unit\'es de \minref{m\'ediocrit\'e} au carr\'e, donc
vous devez assigner une valeur plut\^ot large \`a ce param\`etre (dans les
milliers) pour qu'elle ait un quelconque effet.
\PlainTeX\ mets |\finalhyphendemerits| \`a $5000$.
}
...
\PlainTeX\ mets |\binoppenalty| \`a $700$.
\enddesc
...
\PlainTeX\ mets |\rel!-penal!-ty| \`a~$500$.
...
\li Si \TeX\ doit couper l\`a, il mets le \<pre-break text> just avant
...
Parfois, \TeX\ ins\`ere des coupure optionnelle de lui-m\^eme.
Par exemple, il ins\`ere |\discretionary!allowbreak{}{}{}| apr\`es
...
Son argument est un suite de mots s\'epar\'ee par des blancs. 
Les lettres majuscules et minuscules sont \'equivalent.
Les c\'esures dans chaque mot indique les endroits
...
que vous voulez que \TeX\ coup, c'est-\`a-dire, le singulier et le pluriel.
...
permet d\'ecouper des mots, comme des noms propre,
qui commence par une lettre capital.
Une valeur \`a z\'ero ou n\'egative
inhibe de telle coupure.  \PlainTeX\ mets |\uchyph| \`a $1$,
donc \TeX\  essaye normalement de couper les mots qui commence par une lettre capital.
\enddesc
...
couperait certain jeu de mots al\'eatoires.
Les mots, avec les coupures visibles, apparaissent dans la log et sur
...
\explain
Des langages diff\'erents ont diff\'erent jeux de r\`egles de c\'esure.
...
r\`egles de c\'esure appropri\'ee \`a un langage particulier.
...
pour l'anglais)
et quel sont les valeurs appropri\'ees de |\language|.
La valeur par d\'efaut de |\language| est $0$.

\TeX\ mets le langage courant \`a $0$ au d\'ebut de tous les paragraphes,
...
\explain
Cette commande mets le langage courant \`a \<nombre>
...
qui nommes une police, pas un \<nom de police> qui nommes des fichiers de
police.
Attention~: 
...
^|\font|, il mets le ^|\hyphenchar| de la police dans
|\default!-hyphen!-char|.
...
\PlainTeX\  mets |\default!-hyphen!-char| \`a $45$, le code \ascii\ 
pour `|-|'.
\example
\defaulthyphenchar = `-
...
par de l'espace vertical suppl\'ementaire et le mets en
...
\explain
^^{listes d'\'el\'ements}
Cette commandes est utile pour cr\'eer des ^{listes d'\'el\'ements}.  Tout le paragraphe
...
Elle mets \<argument> en police grasse et le paragraphe suivant en
italiques.  \<arg\-u\-ment> doit \^etre suivi par un point et un token espace,
qui servent \`a s\'eparer \<argument> du \<texte g\'en\'eral>.
\<texte g\'en\'eral> est constitu\'e du texte jusqu'au fronti\`eres du paragraphe suivant,
