...
des \minref{bo\^\i te}s cons\'ecutive d'une \minref{liste verticale} ordinaire,
...
Danq le cas courant, quand les bo\^\i tes ne sont pas anormalement hautes ou profonde, \TeX\
rend la distance de la ligne de base d'une bo\^\i te \`a la ligne de base de la
suivante \'egal \`a |\baselineskip|.  Il fait cela en ins\'erant un ressort inter-ligne
...
il ne doit ins\'erer aucun ressort inter-ligne.} Voir les \knuth{pages~79--80}{99--99} pour une
...
cot\'e.  Voir la \knuth{page~78}{99} pour d'autres d\'etails.  Le premier exemple ci-dessous
...
largeur de cette bo\^\i te. \TeX\ met |\prevdepth| \`a $-10000$\pt\ au d\'ebut d'une
...
\cts normalbaselines {}
\explain
Ces trois param\`etres contienne des valeurs pour |\base!-line!-skip|,
|\line!-skip| et |\line!-skip!-limit| respectivement.
^^|\baselineskip| ^^|\lineskip| ^^|\lineskiplimit|
La commande |\normalbase!-lines| mets |\base!-line!-skip|,
|\line!-skip| et |\line!-skip!-limit| aux valeurs contenues dans les
trois param\`etres.
\enddesc
...
prochaine ligne.
Elle n'a aucun effet sur les lignes suivante.
\enddesc
...
Une commande |\openup| avent la fin d'un paragraphe affecte
tout le paragraphe, donc vous ne devez pas utiliser |\openup| pour
changer |\baseline!-skip| dans un paragraphe.  |\openup| est
plus utile pour composer des tables et des affichages math\'ematique---un
petit espace suppl\'ementaire entre des rang\'es les rend souvent plus lisibles.
...
\cts bigbreak {}
\explain
Ces commandes indique des endroits de plus en plus d\'esirable pour \TeX\ de
couper une page.  Elles demande aussi \`a \TeX\ 
d'ins\'erer un ^|\smallskip|, ^|\medskip| ou
...
\cts supereject {}
\explain
Ces commandes force une coupure de page \`a la position courante
...
ainsi elle sont produites avant que tout autre entr\'ee soit ex\'ecut\'ee. 
...
ait la m\^eme profondeur, c'est-\`a-dire, que leurs marges du bas soient \'egales.
...
qui contient de large blocs de mat\'eriel qui ne doit pas \^etre s\'epar\'e sur deux pages.
...
entre les paragraphes, en esp\'erant que les pages ait assez d'\'etirement pour
...
Ce param\`etre sp\'ecifie la \minref{p\'enalit\'e} pour une 
coupure de page juste avant la derni\`ere ligne d'une paragraphe partiel
...
|\insert|
et d\'ecouvre qu'un insertion pr\'ec\'edente du m\^eme type sur cette page a
\'et\'e s\'epar\'ee, laissant une partie pour les page suivantes.
Voir les \knuth{pages~123--125}{999--999} pour les d\'etails de ce
calcul.
...
routine de sortie---c'est le nombre d'insertions qui ont \'et\'e vu
mais qui ne tiennent pas sur la page courante (voir la \knuth{page~125}{999}).
\enddesc

\begindesc
\cts floatingpenalty {\param{nombre}}
\explain
Ce param\`etre sp\'ecifies la \minref{p\'enalit\`e} que
...
\'et\'e coup\'e, en laissant une partie d'elle pour des pages suivantes.
...
\explain
Ces quatre param\`etres sp\'ecifie ensemble le montant d'\minref{\'etirement}
dans les \minref{ressort} de la page courante.  Le montant d'\'etirement a
la forme
{\def\f#1{\hbox{\tt fi#1}}%
...
\'el\'ements \`a la \minref{liste vertical} principale.
...
il d\'ebut l'impression, comme \'etant \`a un pouce sous le haut de la page et \`a un 
...
si la hauteur de la bo\^\i te est $e$, plus grand que |\topskip|, sa ligne de base 
sera \`a |\topskip|\tplus$e$ du haut de la page.
Voir les \knuth{pages~113--114}{999--999} pour les autres d\'etails  sur le
fonctionnement de |\topskip|.
\PlainTeX\ mets |\topskip| \`a |10pt|.
\enddesc
...
\PlainTeX\ mets |\parskip| \`a |0pt plus 0.1pt|.
\enddesc
...
Sans cette ajustement, la bo\^\i te du bas d'une 
page pourrait bien s'\'etendre dans la marge du bas ou m\^eme recouvrir
la page enti\`erement.  \PlainTeX\ mets |\maxdepth| \`a |4pt|.
\enddesc
...
ou directement dans une hbox qu fait partie de la liste vertical principale,
la marque migrera vers la \minref{liste vertical} principale.
Voir les \knuth{pages~259--260}{999-999} pour des exemples montrant comment |\mark| 
peut \^etre utilis\'e.
\enddesc
...
\TeX\ mets les valeurs de ces commandes quand il
...
La table en bas de la \knuth{page~258}{999} illustre la relation entre
...
il mets les valeurs de ces commandes comme suit~:
\ulist\compact
...
aucune 
\minref{insertion}s (comme d'autres notes de pied de page).
...
Si vous n'\^etes pas sur de la mani\`ere dont ces restrictions s'applique, il 
peut \^etre sage de n'utiliser
|\footnote| et |\vfootnote| que directement dans un paragraphe ou
entre des paragraphes.

Ces restrictions ne sont pas aussi s\'ev\`ere qu'elles ne le semblent parce que 
...
Il y a de rare circonstances o\`u vous devez alt\'erer
...
d\'eplacera le mat\'eriel vers le prochain haut de page possible.
\li |\pageinsert| mets le mat\'eriel lui-m\^eme sur la page suivante.
...
Des insertions sont tr\`es pratique pour du mat\'eriel tel que des figures et des
tables parce que vous pouvez positionner ce mat\'eriel o\`u vous voulez sans
savoir o\`u les coupures de page tomberont.

Chacune de ces commandes terminent implicitement le paragraphe courant, donc 
...
toutes construite autour d'|\insert|.
^^|\topinsert|
^^|\midinsert| ^^|\pageinsert| ^^|\footnote| ^^|\vfootnote|

Quand vous concevez des insertions pour un document, vous devez assigner un diff\'erent
code entier\footnote
...
avec le code $n$.  Quand \TeX\ coupe une page, il mets dans |\box|$\,n$
...
l'inser\-tion.
Voir les \knuth{pages~122--125}{999--999} Pour d'autres d\'etails sur la fa\c con dont \TeX\ 
...
\TeX\ mets la page dans la ^|\box255|, ainsi |\output| est responsable
...
ent\^ete ou des pieds de page.
\enddesc

\begindesc
\cts plainoutput {}
\explain
Cette command invoque la routine de sortie de \plainTeX.
^^{routine de sortie//par d\'efaut dans \plainTeX}
\PlainTeX\ d\'efinit ^|\output| comme
une liste de token contenant le seule et unique token |\plainoutput|.
\enddesc
...
Plus pr\'ecis\'ement, \TeX\ mets |\dead!-cyles| \`a $0$ \`a chaque fois qu'il ex\'ecute
|\ship!-out|
...
\PlainTeX\ mets |\maxdeadcycles| \hbox{\`a $25$}.
\enddesc

\begindesc
\cts outputpenalty {\param{nombre}}
\explain
\TeX\ mets ce param\`etre quand il coupe une page.  Si le point de coupure \'etait sur un
\'el\'ement de \minref{p\'enalit\'e}, \TeX\ enl\`eve l'\'el\'ement de p\'enalit\'e et
mets |\outputpenalty| \`a la valeur de la p\'enalit\'e au point de coupure~; 
autrement, il mets |\outputpenalty| \hbox{\`a $0$}.
...
que \`a besoin de refaire le contenu de la page, par exemple,
...
Le point de division alors correspond \`a la coupure de page qu'il trouverait.
La bo\^\i te $B_2$ doit \^etre une vbox, pas une hbox.
% we avoid starting the previous sentence with a symbol, a copyediting no-no.
\TeX\ mets le mat\'eriel pr\'ec\'edant le point de  division dans
...
\<dimension> parce que \TeX\ peut ne pas \^etre capable de achever son but de page parfaitement.
