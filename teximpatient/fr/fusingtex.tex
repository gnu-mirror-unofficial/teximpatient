...
utiliser la commande ^|\input| (\xref\input) pour inclure un fichier source 
...
standard de \TeX, celui d\'ecrit dans ce livre, inclus une collection de 
...
r\'esultat du driver de p\'eriph\'erique vers l'impri\-mante ou \`a tout autre 
...
diff\'erents dossiers de forme. ^^{magnification} Les fichiers de m\'etrique 
...
programmes compagnon utilisent. Par exemple, |cmr10.tfm| est le dossier de 
...
explicite. Les nombres entre parenth\`eses sont des num\'eros de page que \TeX 
...
quand il en a fini avec le fichier. Si vous recevez n'importe quels 
^{messages d'erreur} dans la sortie affich\'ee, vous pouvez les associer avec 
...
\knuth{chapitre~6}{} et votre ^{information locale}. 
\eix^^{ex\'ecuter \TeX}
...
Un source \TeX\ se compose d'une s\'equence de commandes qui indiquent \`a 
...
Ne faite pas suivre un mot de contr\^ole par le texte qui le suit---\TeX\ ne 
...
\TeX\ se plaindra au sujet d'une \'equence de contr\^ole |\ccon| non d\'efinie.
...
\xrdef{introparms}
Certaines commandes sont des param\`etres (\xref{param\`etre}).
...
qui sont normalement des registres mais se comporte comme des param\`etres.
...
de codes de cat\'egorie (\xref{codes de cat\'egorie}). Ainsi la commande 
...
espaces de largeur variable appel\'ees ``ressort'' (\xref{ressort}), qui ont une 
...
^|\frenchspacing| (\xref\frenchspacing). |\frenchspacing| est souvent 
recom\-mand\'ee pour les ^{bibliographies}.
...
mots compos\'es comme ``will-o'-the-wisp'', les demi-cadratins d\'esignerons 
les suites de page telles que ``pages~81--87'', et les cadratins 
d\'esignerons une coupure dans la continuit\'e---comme ceci.
...
Vous enfermez une formule des textes entre des signes dollar simples 
...
l'estomac accumule des lignes jusqu'\`a ce que plus aucune lignes ne 
puissent rentrer dans la page. Il choisit alors un endroit simple pour 
...
(avant qu'ils ne soient coup\'es en lignes) et le mode vertical ordinaire pour les 
...
sont affih\'ees sur des lignes seules (voir ``formules math\'ematiques'', 
\xref{mathform}).
...
Cette r\'evision inclus quelques fonctions suppl\'ementaires mineures 
...
y compris le \AMSTeX. Son adresse est~: 
\smallskip
{\obeylines
^{\TUG}
...
d\'ecrits dans la \chapterref{eplain} aussi bien que les macros utilis\'e en 
...
\smallskip
\noindent
Un grand merci \`a ^{Jean-C\^ome Charpentier} pour cette traduction tant attendu par la 
...
\smallskip
\noindent
tr\`es bien fait, un must pour comprendre la gestion des polices sous \TeX\ et \LaTeX.
