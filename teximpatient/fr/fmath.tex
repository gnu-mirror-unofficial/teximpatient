...
Les symboles produit par |\backslash|, |\vert| et |\Vert|
...
Les op\'eration binaire sont une des \minref{classe}s des symboles math\'e\-matiques de \TeX.
\TeX\ mets diff\'erent montants d'espace autour de dif\-f\'e\-rentes classes de symboles
...
\emph{que} si la formule doit \^etre coup\'e en ce point.
Il n'y a aucun point en utilisant |\*| dans des formule affich\'ees \minrefs{math\'ematique 
...
\egroup
\explain
Ces commands produisent des symboles pour diff\'erentes relations.
Les relations sont une des \minref{classe}s des symboles math\'ematiques.
\TeX\ mets diff\'erent montants d'espace autour de diff\'erentes classes de symboles
math\'ematiques.
...
sont deux des \minref{classe}s de symboles math\'ematiques de \TeX.  \TeX\ mets
diff\'erents montants d'espace autour des diff\'erentes \minref{classe}s de symboles math\'ematiques.
Vous devriez vous attendre \`a ce que l'espacement que \TeX\ mets autour des ouvrants et
...
(sous toutes ses forme) en dehors du mode math\'ematique.
...
\egroup}
\explain
Ces commandes mettent les noms de diff\'erentes fonctions math\'ematique
en type romain, comme \`a l'accoutum\'ee.
Si vous appliquez une exposant ou un indice sur une de ces commandes,
...
^{style texte} et les plus grandes taille quand il est en ^{style d'affichage}.
Les Op\'erateurs sont une des \minref{classe}s de symboles math\'ematiques de \TeX.
\TeX\ mets diff\'erents montants d'espace
autour des diff\'erentes classes de symboles math\'ematiques.

Les grand symboles d'op\'erateur avec `|big|' dans leurs noms sont diff\'e\-rents
des op\'erations binaires correspondantes (voir \xref{binops}) comme 
|\cap| ($\cap$) puisqu'il apparaissent habituellement au d\'ebut
...
|\Sigma| et |\Pi| produisent les lettres grecques capitales, qui sont plus petite et
...
places les limites pour |\int| apr\`es l'op\'erateur dans tous les cas.)
^^|\int//limites apr\`es|
Cette commande demande \`a \TeX\ de placer
...
positionn\'e sur la \minref{ligne de base}.  Elles ne sont valide q'en
...
En contrepartie, \TeX\ mets un montant d'espace \'egal des deux cot\'es
...
`|:|' est un op\'erateur, donc \TeX\ mets une espace suppl\'ementaire \`a sa gauche tandis qu'il
...
colonne composent \<argument> comme une puissance et celle de la seconde
colonne composent \<argument> comme un indice.  Les commandes |\sb| et |\sp|
...
l'englober dans un \minref{groupe}.  \TeX\ ne mets pas de priorit\'e aux puissances
ou aux indices, donc il rejettera des formules telles que |a_i_j|, |a^i^j| ou
|a^i_j|.

les puissances et les indices sont normalement compos\'e en ^{style script} ou
...
Vous pouvez appliquer une puissance ou un indice \`a tous les commandes qui
produisent des fonction math\'ematique nomm\'ees en caract\`ere romain (voir
...
commandes de choix de police telles que |\it|, elle sont effective
...
Voir la \knuth{page~360}{999} pour des exemples d'utilisation de |\mathpalette|
et la \knuth{page~151}{999} pour plus d'explication sur son fonctionnement. 
...
Les autres accents ne sont applicable qu'a des caract\`eres simples.
...
\explain
Ces commande demandent \`a \TeX\ de composer un accent math\'ematique
dont la famille et le code de caract\`ere sont donn\'e par \<mathcode>.  (\TeX\ ignore
...
{\def\fri{\<formula$_1$>}%
\def\frii{\<formula$_2$>}%
Ces commandes empile une sous-formule sur une autre.  Nous expliquerons comment
...
Vous ne pouvez pas utiliser |\over| ou un autre de ces commandes de ce groupe
...
Les autres commandes sont similaire \`a |\over|, avec les exceptions suivantes~:
\ulist\compact
...
|\over|, |\atop| et |\above|. Le \<dimen> dans |\abovewithdelims|
...
\explain
Ces commandes placent des ^{accolades}, des lignes ou des ^{fl\`eches} extensible 
...
accolades, lignes ou fl\`eches qu'il produira
...
les axe (voir l'explication de |\vcenter|, \xref\vcenter).
...
La plus petite taille disponible pour ces d\'elimiteurs est |\Big|.
...
La taille verticale des d\'elimiteur est ajust\'ee pour correspondre \`a la 
taille vertical (hauteur plus profondeur) de \<subformula>.  \<delim$_1$> et
\<delim$_2$> n'ont pas besoin de correspondre.
Par exemple, vous pouvez utiliser `|]|' comme d'un d\'elimiteur gauche
et `|(|' comme d'un d\'elimiteur droite dans une utilisation simple de |\left|
et |\right|.
...
d\'elimiteur sera r\'eduit pour correspondre \`a la taille verticale de la sous-formule.
...
Voir la \knuth{page~152 et la page~446 (R\`egle 19)}{page~999 et la page~999 (R\`egle 19)}
pour les d\'etail exactes sur la fa\c con dont \TeX\ utilise ces param\`etres.
\PlainTeX\ mets |\delimiter!-factor| \`a $901$ et 
...
\minref{alignement}.)
Les commandes diff\`erent de mani\`eres suivantes~:
\ulist\compact
...
\li |\bordermatrix| produit une matrix dans laquelle la premi\`ere rang\'e et la premi\`ere
colonne sont trait\'ees comme des labels.  (Le premier \'el\'ement de la premi\`ere rang\'e est
habituellement laiss\'e \`a blanc.)  Le reste de la matrice est englob\'e dans
des parenth\`eses.
\endulist
\TeX\ peut rendre les parenth\`eses de |\pmatrix| et de |\bordermatrix| aussi large que
...
\explain
Cette commande produit la notation pour un racine de \<argument$_2$> o\`u l'ordre
...
|\eqno| mets le num\'ero d'\'equation \`a droite et |\leqno| le mets \`a gauche.
...
centr\'e ind\'ependamment des autres lignes.
Vous pouvez utiliser la commande |\noalign| (\xref \noalign) pour changer le montant
d'espace entre deux lignes d'un affichage multi-ligne.

Si vous voulez attacher des num\'ero d'\'equation \`a certaine ou toutes les \'equations
...
|\eqalignno| mets les num\'eros d'\'equation \`a droite et
|\leqalignno| les mets \`a gauche.

Chaque ligne dans l'affichage est termin\'ees par |\cr|.  Chacune des parties devant \^etre align\'ees
(le plus souvent un signe \'egal) est pr\'ec\'ed\'e par
`|&|'.  Un `|&|' pr\'ec\`ede aussi caque num\'ero d'\'equation, qui est donn\'e \`a la fin d'une ligne.
Vous pouvez mettre plus d'une de ces commandes dans un seul affichage pour
produire plusieurs groupes d'\'equation.  Dans ce cas, seuls les groupes les plus \`a droites
ou les plus \`a gauche peuvent \^etre produit avec |\eqalignno| ou |\leqalignno|.

Vous pouvez utiliser la commande |\noalign| (\xref \noalign) pour chager le montant
d'espace entre deux lignes d'un affichage multi-ligne.
\example
...
\xrdef{mathfonts}
%
\easy\ctsx cal {utilise un police calligraphique majuscule}
\ctsx mit {utilise un police math\'ematique italique}
\ctsx oldstyle {utilise une police de chiffres elz\'eviriens}
\explain
Ces commandes demande \`a \TeX\ de composer le texte suivant dans la police
...
\minref{mode math\'ematique}.  Leur principale utilisation est dans le d\'efinition des commandes
...
en utilisant la ^^{classe} de famille de police donn\'e dans son \minref{mathcode}.
...
Si la valeur de |\fam| n'est pas dans cette interval, \TeX\ utilise la famille du
mathcode du caract\`ere comme dans le cas ordinaire.
\TeX\ mets |\fam| \`a $-1$ \`a chaque fois qu'il entre en mode math\'ematique.
En ddehors du mode math\'ematique, |\fam| n'a pas d'effet.

En assignant une valeur \`a
|\fam| vous pouvez changer la fa\c con dont \TeX\ compose des caract\`eres ordinaire
...
%==========================================================================
\subsection {Rendre des d\'elimiteurs plus grand}
...
Ces commandes rendent des \minref{d\'elimiteur}s plus grand que leur taille normale.
...
\li |\bigr| produces un symbole fermant.
\li |\bigm| produces un symbole de relation.
\endulist
\noindent
...
\cts bracevert {}
\explain
Ces commandes produisent des portions de certain grand
d\'elimiteurs
^^{d\'elimiteurs//parties de}
qui peuvent eux-m\^eme \^etre utilis\'es comme d\'elimiteurs.
Elles font r\'ef\'erence aux caract\`eres de la police math\'ematique ^|cmex10|.
\example
...
d\'efini dans le fichier de m\'etrique de la police, d\'etermine les positions
des accents math\'ematiques. Cela dit, supposez que \TeX\ applique un accent math\'ematique
au caract\`ere `|x|'.  \TeX\ regarde si la paire de caract\`ere
...
math\'ematiques (qui d\'etermine beaucoup d'autres choses) se trouve dans l'\knuth{annexe~G}{}.

Si la valeur de |\skewchar| n'est pas dans l'interval $0$--$255$,
...
restaurer sa valeur original explicitement.
\enddesc
...
commande ^|\font|, il mets le ^|\skewchar| de la police
dans |\default!-skewchar|.
Si la valeur de |\default!-skewchar| n'est 
pas dans l'interval $0$--$255$, \TeX\ n'assigne aucun caract\`ere oblique
par d\'efaut.
\PlainTeX\ mets |\defaultskewchar| \`a $-1$, et il est normalement pr\'ef\'erable
de le laisser ainsi.
...
en respectant l'axes de la formule qu'il est en train de construire.

La premi\`ere forme de la commande
centre le mat\'eriel comme donn\'e.  Les deuxi\`emes et troisi\`emes
forme \'elargissent ou r\'etr\'ecissent le mat\'eriel verticalement comme dans la 
...
\explain
Ce param\`etre d\'efini une distance \'egale \`a trois points (\`a moins que
vous le changiez).
...
Voir les \knuth{pages~188--189}{999--999} pour une explication plus d\'etaill\'ee de ce param\`etre.
\enddesc
...
Voir les \knuth{pages~188--189}{999--999} pour une explication plus d\'etail\-l\'ee de ce param\`etre.
\enddesc
...
ins\`ere avant et apr\`es une formule math\'ematique en mode texte (c'est-\`a-dire, un formule
entour\'ee de |$| simples).  Voir la \knuth{page~162}{999} pour plus de d\'etails sur
son comportement.
...
\`a une ^{classe} particuli\`ere (voir la \knuth{page~154}{999} pour la d\'efinition
