...
\bix^^{tables}
Un \defterm{alignement} est une construction pour aligner du mat\'e\-riel tels
...
Les alignements tabul\'es sont d\'efinit par \plainTeX. Ils sont plus simples
...
diviser l'es\-pace horizontal disponible en colonne. Ensuite vous procurez
...
vous pouvez mettre autres choses entre les rang\'ees d'aligne\-ment tabul\'e ou
d\'ecrire plusieurs alignement tabul\'e avec un seul |\settabs|. Voici un exemple
...
colonne plut\^ot que comme une s\'erie de rang\'ee. Un alignement vertical suit
...
caract\`ere.  Une s\'equence contr\^ole commence toujours par un \refterm{caract\`ere
d'\'echappement}.  Notez que les espaces et les fins de ligne sont des caract\`eres 
de plein droit, de plus \TeX\ compresse une suite d'espace en un seul token
d'espace.  Voir \knuth{pages~46--47}{99--99} pour les r\`egles avec lesquelles \TeX\ 
...
constructions similaires qu'il trouve (voir \knuth{pages~212--216}{999--999}) et passe
...
d\'evelopp\'es.  {\TeX} parcoure ce d\'eveloppement de gauche \`a droite \`a moins 
...
couper une suite continues de lignes
...
par d'autres program\-mes.  Il envoi alors la sortie transform\'ee dans le 
\dvifile.
...
les autres. mais la v\'erit\'e en la mati\`ere est que des commandes ex\'ecut\'ees dans l'estomac 
...
suivi par les tokens de texte ordinaires `|pc plus 1em|'
...
d\'elimit\'es et non d\'elimit\'es dans \knuth{pages~203--204}{999--999}.
\endconcept
...
ont \'et\'e standardis\'es. Vous pouvez trouver leurs significations dans une ``table
de codes'' \ascii\ comme celle  de la \knuth{page~367}{999}.
...
\`a une entr\'e dans une de ces tables internes
...
Voir \knuth{pages~276--277}{999--999} pour la syntaxe d\'etaill\'ee des assignements.
\endconcept
...
et donne une \refterm{vbox} ^^{vbox} (bo\^\i te verticale).
^^{listes verticales}
...
lignes-guide claires d'un bloc de papier lign\'e.  
...
\knuth{page~77}{99} pour les~d\'etails.
\par}
...
Voir les \knuth{pages~78 et 80--81}{99 et 99--99} pour les
...
des bo\^\i tes comme celle-ci ne sont pas souvent utiles.
...
\TeX\ obtient ses caract\`eres saisis en les lisant \`a partir fichier source 
(ou de votre terminal) et en d\'eveloppement des \refterm{macros:macro}. Ce
...
seul. Voir les \knuth{pages~46--48}{99--99} pour les d\'etails de ce filtrage.

Les ``^{caract\`eres de contr\^ole}'' \ascii\ ont les codes $0$--$31$ et $127$--$255$.
Ils ne sont pas visualisables ou provoque des choses \'etranges si vous essayez
de les afficher. 
...
d'autres \'el\'ements typographiques et les arrangent sur la page.
...
connaissance de classes de caract\`ere pour d\'ecider combien d'espace mettre
...
\knuth{page~154}{999} pour leurs significations.
...
Pour de si simple applications \TeX\ vous n'avez pas \`a vous pr\'eoccuper des
codes de cat\'egorie, mais ils deviennent importants quand vous essayez de 
terminer des effets sp\'eciaux.
...
\noindent Mise \`a par ceux des cat\'egories $11$--$13$,
...
\catcode`\[ = 1 \catcode`\] = 2
|
Alors les caract\`eres crochets droits et gauches deviennent \'equivalent aux caract\`eres de
...
C'est parce que certains claviers, tout du moins certain \`a 
...
agit comme une commande, sauf celles qui font partie des arguments d'une
...
Quelques exemples de diff\'erentes sortes de commande de \TeX\ sont~: 
\ulist\compact
...
\TeX\ mets entre les paragraphes
...
le nombre total de d\'em\'erites. Voir les \knuth{pages~96--101}{99--999} pour une 
...
la line \`a cet endroit.  Une p\'enalit\'e de $10000$ ou plus emp\^eche une coupure de
...
^|\nobreak| (\pp\xrefn{hbreak},~\xrefn{hnobreak}).
...
assignant des valeurs diff\'erentes aux \refterm{param\`etres:param\`etre} de coupures de lignes de \TeX. 
...
Vous pouvez contr\^oler le choix de coupure de page de \TeX\ de plusieurs fa\c con~:
\ulist
...
Il appelle alors la \refterm{routine de sortie} courante pour ex\'ecute |\box255| 
...
vous \^etre plus familier. Les ^{unit\'es de mesure} ind\'ependante des polices
...
Deux unit\'es de mesure suppl\'ementaire sont associ\'ees avec chaque police~: `^|ex|',
...
dimension sur la \knuth{page~270}{999}.  Voici quelques exemples de dimensions~:
...
d\'efini par d\'efaut. Il d\'efinit une page de $8 \frac1/2$
...
Les $22$ d\'elimiteurs \plainTeX, montr\'e \`a leur taille normale, sont~:
\display{%
$( \>) \>[ \>] \>\{ \>\}
...
Ils sont list\'es \`a un seul endroit sur la \knuth{page~146}{999}.
...
code de d\'elimiteur non n\'egatif
...
(voir les \knuth{pages~156 et 442}{999 et 999}).
Une compr\'ehension compl\`ete de ces r\`egles requi\`ere des connaissances
...
voir les \knuth{pages~97--98}{99--99} pour les d\'etails sur comment \TeX\
...
comme une \refterm{bo\^\i te} de largeur, hauteur et profondeur \`a z\'ero---en d'autre
termes, une bo\^\i te qui ne contient rien et n'occupe pas d'espace.

Trois sortes d'\'el\'ements extraordinaires sont construit dans \TeX~: 
\ulist
\li Les commandes |\openout|, |\closeout| et |\write|
(\p\xrefn{\openout})
% (2nd) removed \xref to \write, since it's on the same page
produit un \'el\'ement extraordinaire pour agir sur un fichier de sortie.
^^|\openout//\elementextra\ produit par|
^^|\write//\elementextra\ produit par|
^^|\closeout//\elementextra\ produit par|
\TeX\ diff\`ere l'op\'eration jusqu'a ce qu'il envoi la prochaine page
dans le {\dvifile}
^^{\dvifile//\elementextra\ en}
(\`a moins que l'op\'eration soit pr\'ec\'ed\'ee par ^|\immediate|).
\TeX\ utilise un \'el\'ement extraordinaire pour ces commandes parce que 
elles n'ont rien \`a faire avec ce qu'il compose quand il les rencontre.
...
qu'il envoi la prochaine page dans le {\dvifile}
...
votre sortie finale.
\li Quand vous changez de langages avec les commandes ^|\language| ou ^|\setlanguage|
\ctsref{\language},
...
Il est aussi possible de produire des ent\^ete multiligne~; voir \xrefpg{bighead} pour
voir comment faire.
\endconcept
...
que vous sp\'ecifie dans la commande.  Le ressort peut
...
une coupure de ligne ou de page (\`a moins d'\^etre suivi imm\'ediatement par un 
...
Normalement vous fixerez les trois polices d'une famille \`a diff\'erents 
taille du m\^eme type de police, mais rien ne vous emp\^eche d'utiliser des 
...
ex\'ecution de \TeX.  \TeX\ manipule des fichiers dans quatre contextes diff\'erent~:
\olist
...
incluant tout messages d'erreur que \TeX\ a g\'en\'er\'e.
...
Vous pouvez avoir jusqu'a $16$ flots d'entr\'ee actif 
\`a la fois, num\'erot\'e de $0$ \`a $15$. 
...
donc `|\read16|', disons, lit le prochaine ligne que vous saisissez sur le 
terminal.
...
\`a la fois, num\'erot\'e de $0$ \`a $15$.  
les flots d'entr\'ee et de sortie sont ind\'ependant.
...
Ainsi `|\write16|', disons, \'ecrit une ligne sur le terminal et aussi envoie
...
est que vous pouvez le r\'eutiliser le flot pour un autre fichier.
Ceci peut \^etre essentiel quand vous lisez un longue suite de fichiers.

De plus vous pouvez assigner des num\'eros vous-m\^eme aux flot d'entr\'ee et de sortie,
...
(voir la \knuth{page~227}{999}
pour les d\'etails).  Cette propri\'et\'e s'applique m\^eme aux messages \'ecrit sur le
terminal avec |\write|.  Mais vous pouvez demande \`a \TeX\ pour faire une action
...
continuer dans le m\^eme \'etat dans lequel \'etait \TeX\ au moment du dump.
...
Dans certain environnements, |tex| est \'equivalent \`a appeler |virtex|
...
La dimension vertical du rectangle
...
`|{|' et `|}|' (ou tout autre caract\`eres avec le 
\refterm{code de cat\'egorie} appropri\'e).
^^|{//d\'ebuter un groupe|
^^|}//terminer un groupe|

La plus importante propri\'et\'e d'un groupe est que tout d\'efinition
...
construit en plaquant les articles d'une \refterm{liste horizontale} l'une 
...
par insertion et le mat\'eriel qui ne passe pas est transf\'erer sur la page
suivante.
Cette proc\'edure autorise \TeX\ de faire des choses comme distribuer des parties 
d'une longue note de pied de page sur plusieurs pages cons\'ecutives.}. Des
exemple de telles insertions sont les notes de pied de page et les figures. 
...
voir les \knuth{pages~122--125}{999--999} pour les d\'etails.
\endconcept
...
les produisent. (Vous pouvez emp\^echer la ligature d'``office'', comme nous
l'avons dit plut\^ot, en saisissant `|of{f}ice|' dans votre entr\'ee.)
...
que vous \'ecrivez a
une lignes de base implicite.
Pour aligner les lettres horizontalement,
vous les placez sur le bloc pour que leur lignes de base
s'ajuste avec les lignes-guides claires imprim\'ees sur le bloc.
...
qu'il a une descendante (la boucle du dessous du `g').
\endconcept
...
dans le texte de remplacement.  Le texte r\'esultant peut contenir d'autre 
...
aider \`a le comprendre, nous vous donnerons un exemple de \TeX\ au travail.
...
d\'elimit\'es est que \TeX\ trouve l'argument correspondant en recherchant
...
Le d\'elimiteur s'\'etend jusqu'au prochains `|#|' ou `|{|'---qui ont un sens
...
d\'elimit\'e est d\'esign\'e par n'importe que nombre de tokens, m\^eme aucun.
...
ne les consid\`erent pas d\'elimit\'es.  Donc si vous \'ecrivez~:
\csdisplay
...
une caract\`ere fin de ligne d\'elimite aussi normalement l'argument
...
alors faire \`a propos des param\`etres de la seconde macro sans mettre \TeX\
...
a moins que la police soit construite dans votre imprimante et que votre driver
...
Chaque changements que vous faites \`a |\hoffset|, |\voffset| ou |\vsize| prendra
...
|
^^|\mark|
Cette macro, quand elle est appel\'ee, produira un ent\^ete de section en caract\`ere gras et
...
Les cas sp\'eciaux, par exemple, pas de sections commen\c cant sur une page, 
s'affichera g\'en\'eralement correctement gr\^ace \`a la mani\`ere dont travaillent
...
Voir les \knuth{pages~258--260}{999--999} pour une explication plus pr\'ecise sur la
...
d'information sur un caract\`ere~: sa position dans la \refterm{police}, sa
...
dans une formule (voir la \knuth{page~154}{999}).  Par exemple, le signe \'egal `|=|'
est dans la classe $3$ (Relation).  \TeX\ utilise sa connaissance des classes 
de caract\`ere quand il d\'ecide combien d'espace mettre entre diff\'erent
composants d'une formule math\'ematique.

La meilleur fa\c con de comprendre tout ce que sont les mathcodes est de voit comment
\TeX\ les utilise. Donc nous vous montrerons ce que \TeX\ fait avec un
...
tant qu'il n'a pas vu tout la formule contenant le
...
\ttidxref{$$}
\TeX\ lit es math\'ematiques affich\'ees dans le \refterm{mode} math\'ematiques affich\'ees.
\endconcept
...
inter-mot ^^{espacement inter-mots} de la ligne d\'evient de leur valeurs 
...
plus grand que $10000$ comme \'etant \'egal \`a $10000$.
...
il acceptera \'eventuellement des lignes dont la m\'ediocrit\'e est inf\'erieur ou \'egal \`a
...
pas d'autre moyen d\'ecouper le paragraphe en lignes.
\li En assumant que toutes les lignes sont tol\'erablement mauvaise, \TeX\ utilise la m\'ediocrit\'e des lignes
...
\TeX\ alors
coupe le paragraphe en lignes d'une fa\c con qui minimise les 
le total des d\'em\'erites pour le paragraphe.
Plus
souvent \TeX\ arrange le paragraphe pour minimiser la m\'ediocrit\'e de la
ligne la pire.  Voir \knuth{pages~97--98}{99--99} pour les d\'etails de comment \TeX\
coupe un paragraphe en lignes.  
\endolist

La proc\'edure de \TeX\ pour assembler une s\'equence de lignes et autres mati\`ere en modes verticaux
...
De plus, quand \TeX\ choisi des coupures de ligne il en
...
Voir \knuth{pages~111--113}{999--999} pour les d\'etails sur comment \TeX\ choisi ses
coupures de page.
\endconcept
...
Parce que vous pouvez encastrer une sorte d'entit\'e avec une autre, c'est-\`a-dire, un vbox
...
articles produit par des commandes verticales internes, par exemple, |\vskip|.
^^{listes horizontales//ne peuvent contenir de commandes verticales}

\ulist
\li Si \TeX\ assemble une liste horizontal dans le mode horizontal ordinaire et
...
voir la \knuth{page~286}{999} pour une liste 
...
{\tighten
Un \defterm{mode math\'ematique} est un \refterm{mode} dans lequel est \TeX\ quand il 
construit une formule math\'ematique.  \TeX\ a deux modes math\'ematiques diff\'erent~: Le 
^{mode math\'ematique de texte} pour construire une formule devant \^etre englob\'ee dans une ligne de texte,
et le ^{mode math\'ematique d'affichage} pour construire une formule devant 
appara\^\i tre seule sur une ligne.  Vous indiquez le mode math\'ematique de texte
...
\emph{espace saisis ne compte pas}.  Voir les 
\knuth{pages~290--293}{999--999} pour des d\'etails sur la fa\c con dont \TeX\ r\'eponds aux 
...
Un \defterm{mode ordinaire} est un \refterm{mode} dans lequel est \TeX\ quand
...
Un \defterm{mode restreint} est un \refterm{mode} dans lequel est \TeX\ quand
...
Une liste verticale ne peut pas contenir tous les \'el\'ements produit par 
...
Deux commandes auxquelles vous devez penser en premier qu'elles sont fondamentalement verticale sont
en fait fondamentalement horizontale~: |\valign| \ctsref{\valign} et |\vrule|
\ctsref{\vrule}.
^^|\valign//horizontal par nature|
^^|\vrule//horizontal par nature|
Voir la \knuth{page~283}{999} pour une liste des commandes
...
En particulier, la plupart des impl\'ementations de \TeX\ consid\`ere un nom de fichier 
comme \'etant
...
d'une fin de ligne comma dans `|{\input chapter2!visiblespace}|'.
...
vous en trouverez dans les \knuth{pages~43--44}{99--99}.
...
une commande.
N\'eanmoins, vous pouvez ins\'erer la forme d\'ecimal d'un nombre dans du texte
...
dans tous les contextes \emph{autre} que comme la partie ``facteur'' d'une dimension,
...
Il associe p\'enalit\'es avec plusieurs types de coupure de page peu attirante---une 
...
(voir la \knuth{page~111}{999} pour la formule du co\^ut).  S'il trouve plusieurs
...
n'est pas n\'ecessairement le dernier endroit possible de couper la page.
Des p\'enalit\'es et autres consid\'erations peuvent faire que la coupure de page
ait lieue plus t\^ot. De plus, \TeX\ appr\'ehende les \'el\'ements de la liste verticale
...
d'un page \`a une autre, vous pouvez \^etre s\^ur comme \TeX\ ex\'ecute l'entr\'ee, que
le num\'ero de la page courante refl\`ete la page sur laquelle la sortie correspondante
appara\^\i tra. Voir les \knuth{pages~110--114}{999--999} pour une description compl\`ete
...
Intuitivement, un \defterm{paragraphe} est une suite de ligne de saisie 
...
rencontre une commande fondamentalement horizontal.
En particulier, il est en mode vertical juste apr\`es avoir termine un 
...
Il y a plusieurs types de commande fondamentalement horizontale, mais le
...
Tout autre commande horizontal en mode vertical
font que \TeX\ fait un |\indent| implicite.
...
(dans le cas o\`u la liste vertical englobante est la liste vertical principale).
...
Le terme \defterm{param\`etre} a deux signification diff\'erentes---il peut faire
...
un montant de \refterm{ressort} ou de muglue ou une \refterm{liste de tokens}.
...
Vous trouverez la liste de tous les param\`etres de \TeX\ dans les \knuth{pages~272--275}{999--999}.
...
\`a cet endroit ou encourage \TeX\ de couper la liste ici.  
...
Couper une liste horizontal ordinaire
...
donc des documents qui n'utilise que les am\'enagements de \plainTeX\ peuvent
facilement \^etre transf\'er\'e d'une installation vers une autre sans
difficult\'e.

\PlainTeX\ est constitu\'e des commandes \refterm{primitive} auxquelles se joignent
une large collection de macros et
autres d\'efinitions.  Ces d\'efinitions additionnelles sont donn\'ees dans 
l'\knuth{Annexe~B}{}.  Elles doivent aussi \^etre dans le fichier |plain.tex|
...
La num\'erotation s'accorde normalement avec le num\'erotation ^{\ascii}
pour les caract\`eres existant dans le jeu de caract\`eres \ascii.
...
(a)~vous s\'electionner une autre police, soit (b)~vous terminez le \refterm{groupe},
au cas o\`u, dans lequel vous avez commenc\'e avec la police. Par exemple,
la saisie~:
...
Vous pouvez utiliser \TeX\ avec d'autres police que Computer Modern 
...
Ainsi il inclue la taille de chaque caract\`ere, les ligatures et cr\'enages
qui modifie les suites de caract\`eres adjacent, et ainsi de suite.
Ce que le fichier de m\'etrique n'inclue \emph{pas} sont les informations
...
Par exemple,
|cmbx12.pk| ^^{\pkfile} doit \^etre le fichier de forme de la police |cmbx12| en
format compress\'e, tandis que |cmbx12.gf| ^^{\gffile} semble \^etre le fichier de forme
...
sont constitu\'ee de commandes primitives auxquelles se joignent
...
la bo\^\i te s'\'etends sous sa \refterm{ligne de base}. 
\endconcept
...
\refterm{macro}
que vous pourriez appel\'e dans le groupe.
...
sont g\'en\'eralement utilisable comme registre ``brouillon''.
Aussi \plainTeX\ ne procure qu'un registre brouillon, |\count255|, pour
des compteurs.
Voir les \knuth{pages~122 et 346}{999 et 999} pour des conventions \`a suivre
...
\li comment les copies du mod\`ele doivent \^etre arrang\'ee dans l'espace
\endolist
...
comment les r\'eglures sont arrang\'ee dans l'espace.}
...
de \refterm{macro} pour pouvoir les utiliser facilement \`a deux endroit. 
...
exacte de la largeur de la r\'eglure.
La diff\'erence entre les trois commandes est dans leur fa\c con d'arranger les
r\'eglures dans la fen\^etre et comment elle distribuent l'espace perdu.
...
les points des deux ligne soient align\'es verticalement (puisque les 
...
L'effet est habituellement d'\'etaler les r\'eglures une petit peu. l'espace
...
\endolist
\noindent
Voir les \knuth{pages~223--225}{999--999} pour les r\`egles pr\'ecises que \TeX\ utilise
...
Le ressort horizontale appara\^\i t dans des \refterm{listes horizontales: liste horizontale},
tandis que le ressort verticale appara\^\i t dans des \refterm{listes verticales:liste verticale}.
^^{listes horizontales}
^^{listes verticales}
Vous
...
Similairement, il d'assure que les diff\'erentes pages ont la
m\^eme ^{marge du bas}
...
Le meilleur moyen de comprendre ce qui s'\'etire et ce qui se r\'etr\'ecie
...
$35$ unit\'es.  Cela
peut arrivez, par exemple, si le hbox doit occuper une ligne enti\`ere
et que la largeur de la ligne soit de $35$ unit\'es.
Puisque les bo\^\i tes ne peuvent changer leurs largeur, 
\TeX\ produit l'espace suppl\'ementaire n\'ecessaire en rendant les ressorts plus large.
L'image maintenant ressemble \`a ceci~:
\gluepicture 
   35 {\picbox 4 \gluebox 6 4 2 8 \picbox 5 \gluebox 10 8 6 14 \picbox 4 }
Les ressorts ne deviennent pas plus large de mani\`ere \'egale~; 
elle deviennent plus large en proportion de leur \'etirement.  
...
$25$ unit\'es.  Dans ce cas \TeX\ fait se r\'etr\'ecir les ressorts.
...
d'un distance inf\'erieure au montant de r\'etr\'ecissement donn\'e dans sa d\'efinition
...
et de m\^eme pour les autres sortes de ressort \'etirable infiniment.
...
Ces deux ressort diviseront tout l'espace suppl\'ementaire de la ligne
\'egalement entre eux.
...
Notez que |fil|, etc., ne peuvent \^etre utilis\'e que pour sp\'ecifier l'\'etirement 
et le r\'etr\'ecissement du ressort---Ils ne peuvent \^etre utilis\'e pour sp\'ecifier 
sa taille normale.
\eix^^{\etirement}
...
La routine de sortie est aussi responsable de collecter 
...
Une \defterm{S\'equence de contr\^ole} est un nom pour une \refterm{command} \TeX.
Une control S\'equence de contr\^ole commence par un ^{caract\`ere d'\'echappement}, 
...
est une s\'equence de   contr\^ole  constitu\'ee d'une
...
Le caract\`ere apr\`es un symbole de  contr\^ole  n'est jamais absorb\'ee par le
symbole de contr\^ole.
\endulist
\noindent Voir \xrefpg{espace} pour plus d'information sur les espaces apr\`es 
des s\'equences de contr\^ole. 

\TeX\ procure une grand nombre de s\'equences de  contr\^ole  pr\'ed\'efinie.  Les
s\'equences de  contr\^ole  \refterm{primitives} sont construite dans le programme 
informatique \TeX\ et donc, sont accessible sous toutes les formes de \TeX.
...
votre facilit\'e informatique peut vous rendre capable de cr\'eer une collection
...
Les struts sont pratique pour obtenir un espacement vertical uniforme quand 
...
Les quatre autres styles sont dit ^{variantes \'etroites}.  Dans ces variantes 
les exposants ne sont pas mont\'es aussi haut que normalement et donc la formule
n\'ecessite moins d'espace vertical qu'ils le devraient.  Voir les \knuth{pages~140--141}{999--999} 
pour les d\'etails sur la fa\c con de \TeX\ de s\'electionner le style.

\TeX\ choisi une taille de caract\`ere en fonction du style~:
...
Voir \conceptcit{famille} pour plus d'informations \`a propos de ces trois taille.
...
est constitu\'e d'un \refterm{caract\`ere d'\'echappement} suivi d'un caract\`ere 
...
indices et les exposants, aussi bine que pour les num\'erateurs et les 
...
La \defterm{taille texte} d\'ecrit la grande des trois 
\refterm{polices:police} li\'ees dans une \refterm{famille}.
^^{famille//taille texte en}
\TeX\ utilise la taille texte pour des symbole ordinaire apparaissant en 
...
Les tests conditionnel sont interpr\'et\'es dans l'\oesophage de \TeX
\seeconcept{\anatomy}, donc tous les \minref{token}s d\'eveloppable dans
le texte interpr\'et\'e ne sont d\'evelopp\'e qu'apr\`es que le test n'ait \'et\'e r\'esolu.
Les diff\'erents tests conditionnels sont expliqu\'e dans 
...
Bien que les documents produits par \TeX\ soit justifi\'e par d\'efaut,  vous
pouvez si vous voulez produire des documents (ou des suites de lignes) qui
ont la marge ^{droite d\'echir\'ee}---ou ^{gauche d\'echir\'ee}.
...
d'\'etirer les ressorts de chaque ligne pour que les marges soit align\'ees.
...
entour\'ee de |$|.
\ttidxref{$}
\TeX\ mets du texte math\'ematique dans le \refterm{mode} texte math\'ematique .
\endconcept
